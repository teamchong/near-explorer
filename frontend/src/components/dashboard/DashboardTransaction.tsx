...
const DashboardTransaction: React.FC = React.memo(() => {
  ...
  const recentTransactionsCountSubscription = useSubscription([
    "recentTransactionsCount",
  ]);
  const latestGasPriceSubscription = useSubscription(["latestGasPrice"]);
      ...
      <TransactionCardNumber>
        <Col xs="12" md="4">
          <LongCardCell
            ...
            loading={recentTransactionsCountSubscription.status === "loading"}
            text={
              recentTransactionsCountSubscription.status === "success"
                ? recentTransactionsCountSubscription.data.toLocaleString()
                : null
            }
          />
        </Col>
        <Col xs="12" md="8">
          <LongCardCell
            ...
            loading={latestGasPriceSubscription.status === "loading"}
            text={
              latestGasPriceSubscription.status === "success" ? (
                <GasPrice gasPrice={latestGasPriceSubscription.data} />
              ) : undefined
            }
          />
        </Col>
